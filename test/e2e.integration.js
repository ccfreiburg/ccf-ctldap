...
function searchWrapper(search, expectEntries, done, options) {
  const entries = [];
  const searchOptions = (options || { scope: 'base' });
...
describe('LDAP Client Server E2E Tests (e2e.integration)', () => {
  before(async () => {
    ...
    await main.start(
      ...
      (site) => async (u, p) => p === 'alex', // pasword check mock
      (ip, port) => {},
    );
  ...
  it('Get dc - query base dn like nextcloud for users', (done) => {
    boundSearchWrapper(
      ...
      { scope: 'sub', filter: '(&(objectclass=nextclouduser)(displayname=*))' },
    );
  });
  ...
  xit('Get dc - query throws', (done) => {
    searchWrapper('cn=eroor,ou=error,dc=error,o=error', () => {}, done);
  });
